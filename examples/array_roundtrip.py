...
# filters=[250] to use plugin_example
cparams = {"filters": [250], "filters_meta": [0], "typesize": 4, "nthreads": 1}  # Compression parameters
dparams = {"nthreads": 1}  # Decompression parameters

# Create blosc2 NDArray from a NumPy array
shape = [100, 100]
size = int(np.prod(shape))
nparray = np.arange(size, dtype=np.int32).reshape(shape)
blosc2_array = blosc2.asarray(nparray, cparams=cparams, dparams=dparams)
